...
namespace PracticeFusion.MmeCalculator.WebDemo.Controllers
{
    ...
    public class CalculatorController : ControllerBase
    {
        ...
        public CalculatorController(ILogger<CalculatorController> logger, ICalculator calculator,
            TransparencyUtils transparencyUtils)
        {
            _logger = logger;
        ...
        public CalculatedResult Get(string rxCui, string sig)
        {
            ...
            {
                CalculationItems =
                    new List<CalculationItem>(new[]
                    {
                        new CalculationItem
                        {
                            RequestItemId = "1",
                            RxCui = rxCui,
                            Sig = HttpUtility.UrlDecode(HttpUtility.UrlDecode(sig))
                        }
                    })
            };
        ...
        {
            _logger.LogInformation("Calculation request: {calculationRequest}",
                JsonUtils.Serialize(calculationRequest, false));
        ...
        ///     GET /api/calculator?sig=1 tablet every 8 hours for 3 days, then 1 tablet every 12 hours for 2 days, then 1 tablet a
        ///     day prn pain
        /// </remarks>
        ...
        public ParsedSig ParseSig(string sig)
        {
            ...
            ParsedSig parsedSig = _calculator.ParseSig(decodedSig);
            return parsedSig;
        }
        ...
        public ParsedSig ParseSigStrict(string sig)
        {
            ...
            var decodedSig = HttpUtility.UrlDecode(HttpUtility.UrlDecode(sig));
            _logger.LogInformation("Url decoded sig: {decodedSig}", decodedSig);
            ParsedSig parsedSig = _calculator.ParseSigStrict(decodedSig);
            return parsedSig;
        }
