...
namespace PracticeFusion.MmeCalculator.RxNavRxNormResolver
{
    ...
    public class Client : IRxNormInformationResolver
    {
        ...
        {
            AllPropertiesResponse response = SearchForDrugByRxNormCui(rxNormCui);
            return response.PropertyConceptGroup?.PropertyConcepts
                ?.FirstOrDefault(x => x.Category == "NAMES" && x.Name == "RxNorm Name")
                ?.Value ?? string.Empty;
        }
        ...
        private void LogRequestResponse(long elapsedMilliseconds, RestRequest request, RestResponse? response,
            string? cachedContent = null)
        {
            ...
            var logRequest = new
            {
                ...
                parameters = request.Parameters.Select(
                    parameter => new
                    {
                        name = parameter.Name, value = parameter.Value, type = parameter.Type.ToString()
                    }),
            ...
            else
            {
                ...
                _logger?.LogDebug("Cached Content: {cachedContent}",
                    logContent == null ? "NULL" : JsonUtils.JsonFormat(logContent));
            }
        }
    }
}