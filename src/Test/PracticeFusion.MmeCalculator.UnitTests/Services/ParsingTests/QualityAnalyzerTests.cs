...
namespace PracticeFusion.MmeCalculator.UnitTests.Services.ParsingTests
{
    ...
    public class QualityAnalyzerTests
    {
        ...
        [DataTestMethod]
        [DynamicData(nameof(ReadTestDataFromFile), DynamicDataSourceType.Method,
            DynamicDataDisplayName = "DisplayName")]
        public void QualityAnalyzerTest(TestMedicationList t)
        {
            // construct the calculator request
            var request = new CalculationRequest { RequestId = t.Id };
            foreach (TestItem testItem in t.TestItems)
            {
        ...
        public static string DisplayName(MethodInfo methodInfo, object[] data)
        {
            ...
            {
                return
                    $"{methodInfo.Name} ({(data[0] as TestMedicationList)?.Description} {(data[0] as TestMedicationList)?.Id})";
            }
        ...
        {
            var testFile = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location),
                "CalculationRequests.txt");
            IEnumerable<string> calculationRequests = File.ReadLines(testFile);
            ...
            foreach (var line in calculationRequests)
            {
                ...
                if (parts.Length > 4)
                {
                    var testItem = new TestItem(parts);
