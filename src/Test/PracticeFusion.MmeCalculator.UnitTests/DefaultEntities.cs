...
namespace PracticeFusion.MmeCalculator.UnitTests
{
    internal class DefaultEntities
    {
        ...
        {
            MinDose = 2, MaxDose = 2, Complex = false, DoseUnit = new DoseUnit { Form = Form(FormEnum.Tablet) }
        };
        ...
        {
            MinDose = 1, MaxDose = 1, Complex = false, DoseUnit = new DoseUnit { Form = Form(FormEnum.Tablet) }
        };
        ...
        {
            IndicationPrecursor = "as needed for", Indication = "chronic pain"
        };

        public static MmeCalculatorResult MmeCalculatorResult => new()
        {
            MaximumMmePerDay = 22.50m, OpioidConversionFactor = 1.5m, OpioidMaximumDailyDose = 15.0m
        };

        public static Form Form(FormEnum form)
        {
            return new Form { ValueEnums = new List<FormEnum> { form } };
        }

        public static Opioid Opioid(OpioidEnum opioid)
        {
            return new Opioid { ValueEnum = opioid };
        }

        public static UnitOfMeasure UnitOfMeasure(UnitOfMeasureEnum uom)
        {
            return new UnitOfMeasure { ValueEnum = uom };
        }
    }
}